...
def movielookup(mykey, searchstring, movietype):
    """Interactions with OMDB API
       mykey = omdb api key
       searchstring = string to search for
       movietype = movie, series, or episode"""
    try:
    ...
    while True:
        ...
        if answer in ["1", "2"]:
            ...
            if answer == "1":
                ...
                searchstring = input("Enter type from available list of choices: movie, series, episode. ")
                type = series
                resp = movielookup(mykey, searchstring, type)
            if resp:
                # display the results
